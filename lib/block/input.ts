...
export type ArrayBlockInputField<Key extends string = string> = CommonBlockInputField<Key> & {
  ...
  typeOptions?: {
    ...
    maxItems?: number;
    /**
     * Ключ одного из полей внутри `properties`, значение которого отображается
     * в заголовке элемента массива.
     *
     * При добавлении нового элемента это поле автоматически заполняется значением,
     * равным `primaryKey`. Если такое значение уже есть среди элементов, к нему
     * добавляется индекс в формате `(N)` (например `name`, `name (2)`, `name (3)`).
     */
    primaryKey?: string;
  };
};
